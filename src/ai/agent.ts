...
7e. POINT-FIRST DERIVATION (Skeleton-to-Solid Principle): Always prefer constructing shapes from underlying skeletal points and 2D curves over placing unanchored atomic 3D solid primitives.
- Primitive to Skeleton-Derived Chain Lookup Table:
  * Box → point / rect → Extrude (or rect + offset_curve → Extrude for hollow walls)
  * Cylinder → point → circle → Extrude (or circle + loft for tapered/flanged columns)
  * Cone → points (base, apex) → circle1 + circle2 → Loft (or revolve triangular profile)
  * Sphere → point → semicircle/arc → Revolve (or Ellipsoid for organic variation)
  * Torus → center point + major/minor radius → Torus (or circle revolved around offset axis)
  * Pipe / Column / Beam → points → Curve (Spline/Line) → Pipe / Sweep
- When atomic primitives are still OK:
  1. Terminal leaves / accent details (e.g. stamen sphere tips, bolt caps, decorative gems).
  2. Instanced prototype elements fed into InstanceOnPoints or CircularPattern.
  3. User explicitly requested a single primitive by name (e.g. "make a red box").
